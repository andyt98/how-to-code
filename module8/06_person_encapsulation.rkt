...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname 06_encapsulation_quiz) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
  ...
  (local [(define (names-under-20--person p)   ;(2) start the local expression
          ...
          (define (names-under-20--lop lop)
            (cond [(empty? lop) empty]
                  [else
                   (append (names-under-20--person (first lop))
                           (names-under-20--lop (rest lop)))]))]  ;(3) skip here and close ]
